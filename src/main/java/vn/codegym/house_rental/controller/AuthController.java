...
public class AuthController {
    ...
    public String registerUser(
        ...
        // Nếu có bất kỳ lỗi nào -> trả về trang đăng ký
        if (bindingResult.hasErrors()) {
            return "auth/register";
        }

        // Lưu người dùng mới
        User newUser = userService.registerUser(register);

        // Tự động đăng nhập sau khi đăng ký thành công
        session.setAttribute(LoginController.SESSION_USER_KEY, newUser);

        // Thông báo thành công & chuyển hướng
        redirectAttributes.addFlashAttribute("successMessage", "Đăng ký tài khoản thành công! Chào mừng bạn đến với HouseRental.");
        return "redirect:/";
    }
}